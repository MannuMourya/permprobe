...
class S3Prober:
    ...
    def run(self, target: Target) -> ResourceReport:
        ...
        # DELETE of a key we never created.
        # 204 = S3 DeleteObject is idempotent when permitted — real signal.
        # 404 NoSuchKey is NOT proof of DeleteObject: AWS masks missing-object
        # 403 as 404 when the caller has s3:ListBucket (documented for GET/HEAD;
        # not confirmed identical for DELETE). Treat as ambiguous.
        ...
        if del_infer.status == 204:
            ...
            del_infer.verdict = Verdict.INFERRED
            del_infer.evidence = (
                "NoSuchKey on canary DELETE — ambiguous: AWS masks missing-object "
                "403s as 404 when the caller has s3:ListBucket, independent of "
                "DeleteObject. Not proof of delete permission on its own."
            )
        ...
        report.warnings.extend(origin_warnings("s3", report.probes))
